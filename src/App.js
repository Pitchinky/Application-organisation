...
function App() {
  ...
  // Timer pour la barre de progression (update chaque minute)
  useEffect(() => {
    const timer = setInterval(() => setNow(new Date()), 60000);
    return () => clearInterval(timer);
  }, []);

  // Déclencheur de chargement des événements
  useEffect(() => {
  ...
  // --- RÉCUPÉRATION DES ÉVÉNEMENTS (CORRIGÉ) ---
  const fetchAllEvents = async () => {
    ...
    // 1. On vide la liste et on lance le chargement
    setEvents([]); 
    setIsLoading(true); 
    ...
    try {
      ...
      // 2. On arrête le chargement quoi qu'il arrive
      setIsLoading(false);
    }
  };
